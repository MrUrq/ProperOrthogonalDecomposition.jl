...

{
    ...
    "category": "section",
    "text": "Compare the singular values from the above two cases with the singular values  from the weighted POD on the equidistance mesh.t, x = range(0, stop=30, length=50), range(-10, stop=30, length=120)\ngrid_resolution = repeat([step(x)],length(x))\n\nXgrid = [i for i in x, j in t]\ntgrid = [j for i in x, j in t]\n\nf1 = sech.(Xgrid.-3.5) .* 10.0 .* cos.(0.5 .*tgrid)\nf2 = cos.(Xgrid) .* 1.0 .* cos.(2.5 .*tgrid)\nf3 = sech.(Xgrid.+5.0) .* 4.0 .* cos.(1.0 .*tgrid)\n\nY = f1+f2+f3\n\nres, singularvals  = POD(Y,grid_resolution)\nnothing # hideAnd the first three singular values.singularvals[1:3]"
},
...

{
    ...
    "category": "method",
    "text": "function modeConvergence(X::AbstractArray, PODfun, stops::AbstractArray{<: AbstractRange}, numModes::Int)\n\nModal convergence check based on l2-norm of modes. The array stops contains the ranges to investigate where stops[end] is used for reference. The numModes largest modes are compared to reduce the computational time. The function used to POD the data is supplied through PODfun\n\n\n\n\n\n"
},
